## Computes MI,TE,TE_C1,TE_C2 using binning
import numpy as np
import pandas as pd
...
def transfer_entropy3(Xall,Yall,xmax,xmin,xbinwidth,ymax,ymin,ybinwidth,k,l):  
    ...
    # For TE_C2
    HXYX1Y1X2 = compute_entropy(np.concatenate((piXX1,piYY1,piX2),axis=1))
    HXYX1Y1 = compute_entropy(np.concatenate((piXX1,piYY1),axis=1))
    HXYX1X2 = compute_entropy(np.concatenate((piXX1,piY,piX2),axis=1))
    HXYX1 = compute_entropy(np.concatenate((piXX1,piY),axis=1))
    ...
    # Compute TE_C2
    TY_X[2] = HXYX1X2 - HXYX1 - HXYX1Y1X2 + HXYX1Y1
    # Compute TE_C1
